    ...
    public playerMovement(dt) {
        ...
        if(this.isdead){
            ...
            if(this.aDown){
                ...
                this.node.scaleX = -2;
                if(!this.anim.getAnimationState('super_man').isPlaying)this.animateState = this.anim.play('super_man');
    ...
    onBeginContact(contact, self, other) {
        if(other.node.name=="Goomba" && !this.isdead ){
            if(this.issmalling || this.isundefeated)contact.enabled = false;
                ...
                if(!this.isSuper && contact.getWorldManifold().normal.y>=0 && ((self.node.y<=other.node.y && !this.isbig)||(this.isbig))){
                    if(this.life==1){
                        //cc.log("gg");
        ...
        else if(other.node.name=="star" && !this.isdead){
            var ori_bgm = this.GAME_MGR.bgm;
            this.isSuper = true;
            cc.audioEngine.playMusic(this.super_music,false);
            this.scheduleOnce(function(){
                cc.audioEngine.playMusic(ori_bgm,false);
                this.isSuper = false;
                cc.log('fFFFF');
            },11)
        }
